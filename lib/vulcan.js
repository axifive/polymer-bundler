...
var Vulcan = function Vulcan(opts) {
  ...
  this.enableCombineCss = Boolean(opts.combineCss);
  this.enableScriptInlining = Boolean(opts.inlineScripts);
  this.enableBundleScripts = Boolean(opts.bundleScripts);
...
Vulcan.prototype = {
  ...
  inlineScripts: function inlineScripts(docObj) {
    var scripts = dom5.queryAll(docObj.doc, matchers.JS_SRC);
    var scriptPromises = scripts.map(function(script) {
      var src = dom5.getAttribute(script, 'src');
      var uri = url.resolve(docObj.href, src);
    ...
    return Promise.all(scriptPromises).then(function(){ return docObj });
  },

  // bundle scripts into one string for saving as bundle.js file
  bundleScripts: function bundleScripts(docObj) {
    var scripts = dom5.queryAll(docObj.doc, matchers.JS);
    if (scripts.length) {
      var body = dom5.query(docObj.doc, matchers.body);
      var script = dom5.constructors.element('script');
      dom5.setAttribute(script, 'src', 'bundle.js');
      dom5.append(body, script);
      var scriptArray = new Array(scripts.length);
      var scriptPromises = scripts.map(function(script, idx) {
        var src = dom5.getAttribute(script, 'src');
        if (src) {
          var uri = url.resolve(docObj.href, src);
          // let the loader handle the requests
          ...
          this.loader.request(uri).then(function(content) {
            if (content) {
              content = encodeString(content);
              scriptArray[idx] = '// ' + src + '\n' + content.trim() + '\n';
            }
          ...
          //scriptArray[idx] = script.childNodes[0].value.trim() + '\n';
          scriptArray[idx] = dom5.getTextContent(script).trim() + '\n';
        }
      }.bind(this));
      return Promise.all(scriptPromises).then(function() {
        docObj.script = scriptArray.join('');
        scripts.forEach(function(script){ dom5.remove(script) });
        return docObj;
      });
    }
    return docObj;
  ...
  //Combine all css into one string for saving as style.css file
  combineCss: function combineCss(docObj) {
    var styles = dom5.queryAll(docObj.doc, matchers.CSS_ALL);
    if (styles.length) {
      var head = dom5.query(docObj.doc, matchers.head);
      var link = dom5.constructors.element('link');
      dom5.setAttribute(link, 'type', 'text/css');
      dom5.setAttribute(link, 'rel', 'stylesheet');
      dom5.setAttribute(link, 'href', 'style.css');
      dom5.append(head, link);
      var styleArray = new Array(styles.length);
      var stylePromises = styles.map(function(style, idx) {
        var src = dom5.getAttribute(style, 'href');
        var media = dom5.getAttribute(style, 'media');
        if (src) {
          var uri = url.resolve(docObj.href, src);
          // let the loader handle the requests
          if (this.isExcludedHref(src)) {
          ...
          return this.loader.request(uri).then(function(content) {
            if (content) {
              ...
              content = this.pathResolver.rewriteURL(uri, docObj.href, content);
              styleArray[idx] = '/* ' + src + ' */\n' + content + '\n';
            }
          }.bind(this));
        } else {
          //var content = style.childNodes[0].value.trim();
          var content = dom5.getTextContent(style).trim();
          if (media) {
            content = '@media ' + media + ' {' + content + '}';
          }
          styleArray[idx] = content + '\n';
        }
      }.bind(this));
      return Promise.all(stylePromises).then(function() {
        docObj.style = styleArray.join('');
        styles.forEach(function(style){ dom5.remove(style) });
        return docObj;
      });
    }
    return docObj;
  ...
  // inline scripts into document, returns a promise resolving to document.
  inlineCss: function inlineCss(docObj) {
    var lastPolymerExternalStyle = null;
    var css_links = dom5.queryAll(docObj.doc, matchers.ALL_CSS_LINK);
          ...
          if (isPolymerExternalStyle) {
            ...
            if (ownerDomModule) {
              ...
              if (polymer2) {
                ...
                content = this.pathResolver.rewriteURL(uri, url.resolve(docObj.href, assetpath), content);
              } else {
                content = this.pathResolver.rewriteURL(uri, docObj.href, content);
              }
              if (!domTemplate) {
    ...
    return Promise.all(cssPromises).then(function(){ return docObj });
  },
  ...
  _process: function _process(target, cb) {
    ...
    if (this.enableBundleScripts) {
      chain = chain.then(function(docObj) {
        return this.bundleScripts(docObj);
      }.bind(this));
    }
    if (this.enableScriptInlining && !this.enableBundleScripts) {
      chain = chain.then(function(docObj) {
        return this.inlineScripts(docObj);
      }.bind(this));
    }
    if (this.enableCombineCss) {
      chain = chain.then(function(docObj) {
        return this.combineCss(docObj);
      }.bind(this));
    }
    if (this.enableCssInlining && !this.enableCombineCss) {
      chain = chain.then(function(docObj) {
        return this.inlineCss(docObj);
      }.bind(this));
    }
    ...
    chain.then(function(docObj) {
      cb(null, dom5.serialize(docObj.doc), docObj.script, docObj.style);
    }).catch(cb);
  },
